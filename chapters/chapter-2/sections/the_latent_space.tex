...
entangled and hard to control. InfoGAN by \cite{chen2016infoganinterpretablerepresentationlearning} introduced an information-maximizing 
objective to encourage disentanglement, yielding interpretable latent factors.
The progressive GAN (PGGAN) of \cite{karras2018progressivegrowinggansimproved} improved image quality via multiresolution training but still used a single latent code at the input. 
A major shift occurred with StyleGAN \cite{karras2019stylebasedgeneratorarchitecturegenerative}, which used a learned mapping network to transform the input z into an intermediate latent W space
This mapping ,via 8 fully-connected layers, "unfolds" the simple z distribution into W, aligning it to the generator's feature distribution and enabling axis-parallel edits\cite{Melnik_2024}.
...
StyleGAN2 built on this by replacing Adaptive Instance Normalization (Ada\\in) with weight modulation and demodulation; it further regularized the latent mapping to smooth out the generator's response to latent shifts \cite{karras2020analyzingimprovingimagequality}.
...
However, $W^+$ vectors do not come from the learned W distribution, so naive sampling can produce realistic "face-like" images but also novel patterns that are still face-like (eg. Aliens) \cite{Melnik_2024}.
...
into a channel-wise affine vector $s$ in Style Space $S$, which directly scales convolutional feature maps \cite{Melnik_2024}.
...
\cite{karras2020analyzingimprovingimagequality} observed that W becomes "much closer to the required feature distribution" than $z$, 
so that semantic edits align with simple directions.
In StyleGAN the $W$ space yields more disentangled representations than the original $Z$ space, making long-range manipulations (e.g. aging) more robust \cite{shen2020interpretinglatentspacegans}.
...
to high-level edits (pose, color, environment) \cite{harkonen2020ganspacediscoveringinterpretablegan}.
...
Similarly, \cite{voynov2020unsuperviseddiscoveryinterpretabledirections} developed an unsupervised optimization to 
...
Semantic Factorization (SeFa) by \cite{shen2021closedformfactorizationlatentsemantics} provides a complementary approach 
...
% Using DCI metrics , \cite{wu2020stylespaceanalysisdisentangledcontrols} compared the $Z$, $W$ and $S$ spaces and found that "while
...
% They also compared it $W^+$ and discovered that $S$ shows higher score. \cite{wu2020stylespaceanalysisdisentangledcontrols} demonstrate that one can find single S-dimensions that locally manipulate "eye color" or "bed appearance" with minimal side effects, far outperforming previous W-based controls.
...
enabling smooth attribute-conditioned sampling and editing \cite{Abdal_2021}.
...
This yields a flexible text-based interface for latent edits ("smile", "pikachu", "gothic") without requiring explicit labeled data. \cite{patashnik2021stylecliptextdrivenmanipulationstylegan}.
...
According to \cite{liu2023delvingstyleganinversionimage}, GAN-Inversion methods suffer from fidelity-editability trade-off where 
...
Modern techniques address this: encoder-based methods like pixel2Style2pixel (pSp) \cite{richardson2021encodingstylestyleganencoder}
or encoder4Editing (e4e) \cite{tov2021designingencoderstyleganimage} quickly predict an approximate $W^+$ code; then post-processing (latent optimization or a "pivotal tuning") refines it.
...
One can generate images with specified attributes by sampling $z$ and then steering $w = M(z)$ in desired directions \cite{patashnik2021stylecliptextdrivenmanipulationstylegan}.
