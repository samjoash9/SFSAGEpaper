...
which was first introduced by \cite{GAN}.
In the paper , "Generative Adversarial Nets" , a generator network $G$ produces samples aimed at mimicking
the data distribution , while the discriminator $D$ estimates the probability of whether an input is real or generated \cite{GAN}.
...
$D$ examines the probability of the the samples provided by $G$ being real or fake \cite{goodfellow2017nips2016tutorialgenerative}.
...
$x$ is a real sample rather than one produced by $G$ \cite{salehi2020generativeadversarialnetworksgans,cohen2022generativeadversarialnetworks,Creswell_2018}.

The adversarial training process in GANs is governed by a minimax optimization. Formally, \cite{GAN} define the value function as
\begin{equation}
\begin{aligned}
...
\end{equation}
Equation~\eqref{eq:gan-value-function} defines the adversarial objective: the discriminator increases the probability of assigning real labels to samples from $p_{\text{data}}$ and fake labels to generated samples $G(z)$, while the generator attempts to reduce this distinction through the minimax optimization in Equation~\eqref{eq:gan-minimax}.
...
while $G$ is trained simultaneously to minimize $\log(1-D(G(z)))$ \cite{GAN}. 
...
\cite{GAN}  suggest a non-saturating heuristic for $G$ (maximizing $\log D(G(z))$) instead of minimizing $\log(1-D(G(z)))$ to provide stronger gradients in practice. 
