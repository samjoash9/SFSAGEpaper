...
\begin{enumerate}
    ...
    \begin{figure}[ht]
        ...
        \caption{\textit{Sample images from the FUNIT Animal Faces dataset. Source: \cite{liu2019few}}}
    ...
    a strong benchmark for generative and attribute-editing models in few-shot settings \cite{liu2019few}.

    \item \textbf{102 Flower (Flowers)}
    \begin{figure}[ht]
        \centering
        \includegraphics[width=0.9\linewidth]{images/flowers_dataset.png}
        \caption{\textit{Sample images from 102 Flower Dataset. Source \cite{Nilsback08}}}
        \label{fig:102Flower}
    \end{figure}
...
\begin{itemize}
    \item Resizing and Normalization: Images are resized according to the generator and evaluation setting, including the reported 1024$\times$1024 experiments where applicable, and pixel values are normalized to the range $[-1,1]$ for input to StyleGAN2.
    \item Filtering: Extremely low-quality or mislabeled images are removed.
    \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item \textbf{Flowers.} We assign 85 categories to the training (seen) split and reserve 17 categories for the test (unseen) split.
    \end{itemize}
    
    \item Augmentation: No data augmentation is used beyond centering and flipping, since the evaluation focuses on the given domains. All datasets and preprocessing pipelines are documented for reproducibility.
\end{itemize}
